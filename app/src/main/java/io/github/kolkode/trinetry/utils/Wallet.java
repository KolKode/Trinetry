...
public class Wallet {
    ...
    public String getKeys(String privateaddr) throws InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchProviderException {
        privateAddress = privateaddr;
        Credentials credentials = Credentials.create(privateaddr);
        publicAddress = credentials.getAddress();
        return publicAddress;
    }
    public void makeKeys() throws InvalidAlgorithmParameterException, NoSuchAlgorithmException, NoSuchProviderException {
        ECKeyPair ecKeyPair = Keys.createEcKeyPair();
        Credentials credentials = Credentials.create(ecKeyPair);
        publicAddress = credentials.getAddress();
        privateAddress = credentials.getEcKeyPair().getPrivateKey().toString(16);
    }
    public void getKeyPairFromSeed(String memonics) {
